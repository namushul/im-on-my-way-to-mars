...
use crate::application::{Request, Server};
use crate::Args;
    ...
    loop {
        ...
        if ends_with_crlf {
            let request = std::str::from_utf8(&buffer[0..total_count])
                .or(Err(Response::bad_request("Failed to parse utf8 string".to_string())))?;

            let url_string = request.split_once("\r\n")
                .map(|pair| pair.0)
                .ok_or(Response::bad_request("Failed to parse utf8 string".to_string()))?;

            let url = url::Url::parse(url_string)
                .or(Err(Response::bad_request("Failed to parse url in request".to_string())))?;

            return Ok(url);
        }

        if total_count == buffer.len() {
            return Err(Response::bad_request("Request too large".to_string()));
        }
...
pub fn handle_connection(stream: &mut SslStream<TcpStream>) -> Response {
    let server = Server {};
            ...
            let peer_fingerprint: Option<[u8; 32]> = match stream.ssl().peer_certificate() {
                Some(peer_certificate) => {
                    match peer_certificate.digest(MessageDigest::sha256()) {
                        ...
                        Err(_) => return Response::temporary_failure("Failed to calculate digest of client certificate".to_string())
                    }
                }
                None => None
            };
            let query = match url.query() {
                Some(query) => {
                    match percent_decode(query.as_bytes()).decode_utf8() {
                        Ok(query) => Some(query.to_string()),
                        Err(_) => return Response::bad_request("Query string contains invalid utf8".to_string())
                    }
                }
                None => None
            };
